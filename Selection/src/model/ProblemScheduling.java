...
public class ProblemScheduling implements Problem {
	...
	public double getFitness(int[] v) { // it would be better to rerun a list or String[], the first value is fitness
										// function, the others are the jobid of selected test cases.
		ArrayList<TestCase> tempCaseList = new ArrayList<TestCase>();

		ArrayList<TestCase> tempCase2 = new ArrayList<TestCase>();

		int count = 0;
		double temptime = 0, priorityValue = 0, probabilityValue = 0, consequenceValue = 0, riskValue = 0,
				ePriorityValue = 0, eProbabilityValue = 0, eConsequenceValue = 0;
		double prioritySum = 0, probabilitySum = 0, consequenceSum = 0, riskSum = 0, ePrioritySum = 0,
				eProbabilitySum = 0, eConsequenceSum = 0;
		double overallProbability = 0, overallPriority = 0, overallConsequence = 0, overallRisk = 0,
				overallEProbability = 0, overallEPriority = 0, overallEConsequence = 0;
		...
		for (int j = 0; j < tempCase2.size(); j++) {
			...
			if (priorityTemp.getName().equalsIgnoreCase("higher") || priorityTemp.getName().equalsIgnoreCase("high")
					|| priorityTemp.getName().equalsIgnoreCase("medium")
					|| priorityTemp.getName().equalsIgnoreCase("low")
					|| priorityTemp.getName().equalsIgnoreCase("lower"))
				priorityValue = convertPriority(priorityTemp);
			else
				priorityValue = 0;

			if (probabilityTemp.getName().equalsIgnoreCase("high")
					|| probabilityTemp.getName().equalsIgnoreCase("medium")
					|| probabilityTemp.getName().equalsIgnoreCase("low"))
				probabilityValue = convertProbability(probabilityTemp);
			else
				probabilityValue = 0;

			if (consequenceTemp.getName().equalsIgnoreCase("higher")
					|| consequenceTemp.getName().equalsIgnoreCase("high")
					|| consequenceTemp.getName().equalsIgnoreCase("medium")
					|| consequenceTemp.getName().equalsIgnoreCase("low")
					|| consequenceTemp.getName().equalsIgnoreCase("lower"))
				consequenceValue = convertConsequence(consequenceTemp);
			else
				consequenceValue = 0;

			if (riskTemp.getName().equalsIgnoreCase("high") || riskTemp.getName().equalsIgnoreCase("medium")
					|| riskTemp.getName().equalsIgnoreCase("low"))
		...
		tm = 1 - (priority * overallPriority + probability * overallProbability + consequence * overallConsequence
				+ 0.3 * time + risk * overallRisk + epriority * overallEPriority + eprobability * overallEProbability
				+ econsequence * overallEConsequence) / 1.3;
