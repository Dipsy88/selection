...
public class SSGA extends Search {
	...
	public void mutateAndEvaluateOffspring(Individual ind) {
		mutateAndEvaluateOffspring(ind, false);
	}

	public void mutateAndEvaluateOffspring(Individual ind, boolean force_muatation) {
		...
		while (!mutated) {
			...
			for (int i = 0; i < ind.v.length; i++) {
				if (p >= RandomGenerator.getGenerator().nextDouble()) {
					...
					while (value == ind.v[i]) {
						final int min = cons[i][0];
						final int max = cons[i][1];
						final int dif = max - min;

						if (cons[i][2] == Problem.CATEGORICAL_TYPE) {
							value = min + RandomGenerator.getGenerator().nextInt(dif + 1);
						} else {
	...
	protected Individual[] xover(Individual a, Individual b) {
		...
		// k is the splitting point. it cannot be the first, because otherwise the
		// offspring
